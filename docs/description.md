...
当前实现包括供应商无关的 Tool Call、Tool Result、Run Status 与 Runtime Event，自研进展感知 Agent
Loop，工作区受限的结构概览、目录浏览、分块文件读取、文本搜索和精确 Patch 工具，以及 TUI
工具过程展示。`apply_patch` 只修改单个现有 UTF-8 文件：Runtime 在审批前准备完整 Diff，用户按
当前 Run 授权，工具通过原子替换提交。Run 结束后 TUI 展示净变更并允许 Review 或整体 Revert；
快照只保存在内存，下一 Run 开始即视为接受，外部修改冲突会阻止 Revert。
`run_command` 让模型根据项目自身的语言、清单和说明选择已有测试、类型检查、Lint 或构建命令，
并将退出码、stdout 和 stderr 作为 Observation 回填。命令以 argv 直接启动，不经过隐式 Shell，
每次都需要审批，并具有工作区 cwd、超时、取消和输出上限。参数校验与真实进程执行采用不同的
Observation 阶段；可确定的 JSON 编码 argv 会被安全归一化，命令非零退出等新诊断结果仍会推动
Agent 继续修复。
只读工具执行前支持允许一次、允许当前会话和拒绝；会话允许项只保存在当前进程内。交互 Run 默认
不消耗固定总迭代或总工具额度，新 Observation 会持续续期；只有重复 Observation 或连续控制层
错误才触发安全总结。一次 Run 只有成功时才会把完整消息链原子提交到当前进程历史。

当前模型能力由 Vortex 项目统一配置：默认模型为 DeepSeek V4 Flash，私有凭证固定从 Vortex
仓库根目录的 `.env` 加载，与启动时选择的 Agent 工作区相互独立。当前阶段不提供用户自带 API Key，
也不会读取业务工作区中的 `.env`。

当前不包含数据库、后台 Core、新建/删除文件、命令 Sandbox 与网络隔离、持久化权限策略、上下文压缩、MCP、
多 Agent 和 Web。
...
当前提供文件读取、目录查看、文本搜索、现有文件精确修改和逐次审批的命令执行。命令工具用于运行
项目已有的验证与只读版本控制检查，不包含隐式 Shell，也不硬编码某一种语言。后续可扩展创建文件、
风险策略和 Sandbox；工具数量保持克制，重点验证统一调用、错误处理和安全边界。
