...
第一阶段优先完成单 Agent 的完整运行闭环、状态持久化、基础工具、权限审批和事件展示。项目不会以模型数量、工具数量或 Agent 角色数量作为早期目标。

## 3. 第一阶段整体架构

Vortex 第一阶段采用模块化单体架构，主要由以下部分组成：
...
Agent Runtime 负责驱动一次任务从开始到结束。第一阶段采用显式状态机组织执行过程，基础状态包括：
...
第一阶段提供少量本地基础工具，例如文件读取、目录查看、受控文件修改、Shell 执行和 Git 状态查看。工具数量保持克制，重点验证统一调用、错误处理和安全边界。

### Context Builder

Context Builder 负责根据当前任务和 Token 预算构造模型输入。第一阶段至少区分：
...
Vortex 定义自己的 `ModelProvider` 抽象，对上层暴露统一的流式响应、工具调用、使用量和结束原因。第一阶段只需要稳定接入一个模型供应商，同时提供可测试的 Fake Provider。
...
第一阶段目标是形成一条完整且可观察的单 Agent 执行链路：
...
这些边界并不限制 Vortex 的长期扩展，而是确保第一阶段能够形成一个结构清晰、运行可靠、可以深入解释的 Agent Runtime 基础。

## 8. 第一阶段完成标准

当 Vortex 能够稳定演示以下场景时，可以认为第一阶段基本成立：
...
第一阶段的成功标准不是 Agent 能自动解决多复杂的问题，而是这条运行链路具有明确的数据模型、稳定的状态转换、可测试的执行行为和可追溯的过程记录。
